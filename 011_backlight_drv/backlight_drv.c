...
#include <linux/backlight.h>                          // backlight header
...
static int sample_platform_driver_probe(struct platform_device *pdev)
{
	printk(KERN_INFO "platform driver probe is called \n");
         bldev = devm_backlight_device_register(&pdev->dev,"ldd-bklight",NULL,NULL, &blops, &blprops);  //backlight device register
	 if(bldev == NULL){
...
static int sample_platform_driver_remove(struct platform_device *pdev)
{
	...
	devm_backlight_device_unregister(&pdev->dev, bldev);                                           //backlight device unregister
	return 0;
}
